...
const ContactItem = ({ icon, value, link, bg }) => (
    <div className={`flex items-center gap-3 hover:scale-105 transition-transform duration-200 ${bg} p-2 rounded-xl shadow-sm`}>
        <div>{icon}</div>
        {link ? (
            <a href={link} className="text-gray-800 hover:underline font-medium" target="_blank" rel="noopener noreferrer">
                {value}
            </a>
        ) : (
                ...
                <div className="max-w-5xl mx-auto p-6 bg-white rounded-2xl shadow-md mt-10 border border-gray-200">
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center">
                        ...
                        <div className="flex-1">
                            <h2 className="text-blue-800 text-2xl font-bold">{lawyer.fullName}</h2>
                    ...
                    <div className="flex flex-row justify-evenly gap-6 bg-white rounded-2xl">
                        <ContactItem icon={<Phone className="text-blue-600" />} value={lawyer.phone} bg="bg-blue-50" />
                        <ContactItem
                        ...
                        <ContactItem
                            ...
                            value="WhatsApp"
                            link={`https://wa.me/${lawyer.whatsapp.replace(/\D/g, '')}`}
                            bg="bg-green-50"
                        />
                    </div>
                    ...
                    <div className="flex flex-row gap-6 justify-evenly text-gray-700 font-medium">
                        <ProfileDetail icon={<MapPin size={18} className="text-blue-500" />} label={lawyer.location} />
                        <ProfileDetail icon={<Briefcase size={18} className="text-blue-500" />} label={`${lawyer.experience} years experience`} />
                        <ProfileDetail icon={<Languages size={18} className="text-blue-500" />} label={lawyer.languages.join(', ')} />
                    </div>
